...
_rnn_persistent_algo = {'standard': None}
...
class BaseNStepRNN(function.Function):
    ...
    def __init__(self, n_layers, states, lengths, rnn_dir, rnn_mode, 
        ...
        # TODO: check cudnn version
        if _cudnn_version < 5000:
            raise ValueError('Please use cudnn v5 >= \
                             if you use this function.')
        if (_cudnn_version >= 5000 and _cudnn_version < 6000 
            and rnn_algo != 'standard'):
            raise ValueError('Please use cudnn v6 >= if \
                             you use Persitent RNN function.')
        ...
        self.use_dynamic_algo = rnn_algo == 'dynamic'
    ...
    def forward(self, inputs):
        ...
        rnn_desc = cudnn.create_rnn_descriptor(
            ...
            self.rnn_mode, libcudnn.CUDNN_DATA_FLOAT, algo=self.rnn_algo)
        ...
        if self.use_dynamic_algo:
            batchsize = len(x_list[0])
            cache_id = str(id(self)) + '_' + str(batchsize)
            cached_rnn_plan = getattr(_thread_local, cache_id, None)
            if cached_rnn_plan is None:
                rnn_plan = cudnn.create_rnn_persistent_rnn_plan(
                    rnn_desc, batchsize, libcudnn.CUDNN_DATA_FLOAT)
                setattr(_thread_local, cache_id, rnn_plan)
            else:
                rnn_plan = cached_rnn_plan
            cudnn.set_rnn_persistent_rnn_plan(rnn_desc, rnn_plan)
...
class NStepRNNTanh(BaseNStepRNN):
    ...
    def __init__(self, n_layers, states, lengths, rnn_algo, **kwargs):
        BaseNStepRNN.__init__(
            ...
            rnn_dir='uni', rnn_mode='rnn_tanh', rnn_algo=rnn_algo, **kwargs)
...
class NStepRNNReLU(BaseNStepRNN):
    ...
    def __init__(self, n_layers, states, lengths, rnn_algo, **kwargs):
        BaseNStepRNN.__init__(
            self, n_layers, states, lengths,
            rnn_dir='uni', rnn_mode='rnn_relu', rnn_algo=rnn_algo, **kwargs)
...
class NStepBiRNNTanh(BaseNStepRNN):
    ...
    def __init__(self, n_layers, states, lengths, rnn_algo, **kwargs):
        BaseNStepRNN.__init__(
            self, n_layers, states, lengths,
            rnn_dir='bi', rnn_mode='rnn_tanh', rnn_algo=rnn_algo, **kwargs)
...
class NStepBiRNNReLU(BaseNStepRNN):
    ...
    def __init__(self, n_layers, states, lengths, rnn_algo, **kwargs):
        BaseNStepRNN.__init__(
            self, n_layers, states, lengths,
            rnn_dir='bi', rnn_mode='rnn_relu', rnn_algo=rnn_algo, **kwargs)
...
def n_step_rnn_base(n_layers, dropout_ratio, hx, ws, bs, xs,
    ...
    if xp is not numpy and chainer.should_use_cudnn('>=auto', 5000):
        ...
        hy, ys = rnn(n_layers, states, lengths, rnn_algo=rnn_algo)(*inputs)
